...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    public function addGroup(GroupStudent $group): self
    {
        if (!$this->groups->contains($group)) {
            $this->groups[] = $group;
            $group->addMember($this); // Ensure the bidirectional relationship is maintained
        }
        return $this;
    }
    ...
    public function removeGroup(GroupStudent $group): self
    {
        if ($this->groups->removeElement($group)) {
            $group->removeMember($this); // Ensure the bidirectional relationship is maintained
        }
        return $this;
    }
    ...
    {
        // Calculate karma based on completed levels
    ...
    public function completeLevel(Level $level): self
    {
        if (!$this->completedLevels->contains($level)) {
            ...
            // Add 1 karma point for each completed level
            $this->addKarmaPoints(1);
            // Recalculate karma points (redundant here since we already calculate in `addKarmaPoints()`)
